...
class Kredit extends CI_Controller
{
    ...
    function data_lb(){
		...
		$data=$this->m_kredit->lb_list($id_lb);
		echo json_encode($data);
	}
        
	function get_lb(){
		$id_lb=$this->input->get('id');
		$data=$this->m_kredit->get_lb_by_kode($id_lb);
		echo json_encode($data);
	}
	...
	function update_lb()
	{
		...
		$hasil = $this->m_kredit->update_lb($id_lb,$tgl_analisa,$tgl_permohonan,$plafon,$jangka_waktu,$sifat_kredit,$suku_bunga,$musiman,$jenis_permohonan
											,$tujuan_permohonan,$ket_penggunaan,$nama_debitur,$status_kawin,$ttl_nasabah,$ktp
											,$alamat_ktp_nasabah,$domisili_nasabah,$hp_nasabah,$status_tt,$pekerjaan_nasabah,$tanggungan
											,$pendidikan,$jenis_kelamin,$masa_laku,$telp_kantor,$lama_tinggal,$nama_pasangan
											,$ttl_pasangan,$alamat_ktp_pasangan,$domisili_pasangan,$pekerjaan_pasangan,$hp_pasangan,$nama_keluarga
											,$hubungan_keluarga,$alamat_keluarga,$hp_keluarga);
		echo json_encode($hasil);
	}
	
	function delete_lb(){
		$id_lb=$this->input->post('kode');
		$data=$this->m_kredit->delete_lb($id_lb);
		echo json_encode($data);
	}
	*/
